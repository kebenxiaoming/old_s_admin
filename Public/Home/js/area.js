...
function show_province(){
    $.post(choose_province,function(result){
        ...
        if(result.error == 0){
            var area_dom = '<select id="choose_province" name="province_id" class="col-sm-1" style="margin-right:10px;width: 86px">';
...
function show_city(id,name,type){
    $.post(choose_city,{id:id,name:name,type:type},function(result){
        result = $.parseJSON(result);
        if(result.error == 0){
            var area_dom = '<select id="choose_city" name="city_id" class="col-sm-1" style="margin-right:10px;width: 86px">';
            $.each(result.list,function(i,item){
...
function show_area(id,name,type){
    $.post(choose_area,{id:id,name:name,type:type},function(result){
        result = $.parseJSON(result);
        if(result.error == 0){
            var area_dom = '<select id="choose_area" name="area_id" class="col-sm-1" style="margin-right:10px;width: 86px;">';
            $.each(result.list,function(i,item){
