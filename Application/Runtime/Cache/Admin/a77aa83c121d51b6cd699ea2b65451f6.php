    ...
    <div id="myTabContent" class="tab-content">
        <div class="tab-pane active in" id="home">
            ...
            <form id="tab" method="post" action="">
                <label>名称</label>
                <input type="text" name="menu_name" value="<?php echo ($currentmenu["menu_name"]); ?>" class="input-xlarge" required="true">
                <label>链接 <span class="label label-important">不可重复</span></label>
                <input type="text" name="menu_url" value="<?php echo ($currentmenu["menu_url"]); ?>" class="input-xlarge" required="true" >
                <label>所属模块</label>
                <select name="module_id" class="input-xlarge" id="DropDownTimezone">
                    <?php foreach($module_options_list as $k=>$val){ if($currentmenu['module_id']==$k){ echo "<option value='".$k."' class='input-xlarge option' id='DropDownTimezone-1' selected='selected'>".$val."</option>"; }else{ echo "<option value='".$k."' class='input-xlarge option' id='DropDownTimezone-1'>".$val."</option>"; } } ?>
                </select>
                <label>是否显示为左侧菜单</label>
                <select name="is_show" class="input-xlarge" >
                    <?php if($currentmenu['is_show']): ?><option value="1" selected >是</option>
                ...
                <select name="father_menu" class="input-xlarge" id="DropDownTimezone">
                    <?php foreach($father_menu_options_list as $k=>$val){ if(is_array($val)){ echo '<optgroup label="'.$k.'">'; foreach($val as $key=>$v){ if($currentmenu['father_menu']==$key){ echo '<option value="'.$key.'" selected="selected" class="input-xlarge option" id="DropDownTimezone-1-0">'.$v.'</option>'; }else{ echo '<option value="'.$key.'" class="input-xlarge option" id="DropDownTimezone-1-0">'.$v.'</option>'; } } echo '</optgroup>'; }else{ if($currentmenu['father_menu']==0){ echo '<option value="'.$k.'" selected="selected" class="input-xlarge option" id="DropDownTimezone-0">'.$val.'</option>'; }else{ echo '<option value="'.$k.'" class="input-xlarge option" id="DropDownTimezone-0">'.$val.'</option>'; } } } ?>
                </select>
                <label>是否有效</label>
                <select name="is_online" class="input-xlarge" >
                    <?php if($currentmenu['is_online']): ?><option value="1" selected>是</option>
                        <option value="0">否</option>
                        <?php else: ?>
                ...
                <select name="shortcut_allowed" class="input-xlarge" >
                    <?php if($currentmenu['shortcut_allowed']): ?><option value="1" selected>是</option>
                    <option value="0">否</option>
                        <?php else: ?>
                        <option value="1" >是</option>
                        <option value="0" selected>否</option><?php endif; ?>
                </select>
                <label>排序 <span class="label label-important">填写数字，越大越靠前</span></label>
                <input type="text" name="sort" value="<?php echo ($currentmenu["sort"]); ?>" class="input-xlarge" placeholder="越小越靠前，默认0"  required="true" >
                <label>描述</label>
                <textarea name="menu_desc" rows="3" class="input-xlarge"><?php echo ($currentmenu["menu_desc"]); ?></textarea>
