...
<div class="block">
    ...
    <div id="page-stats" class="block-body collapse in">
        ...
        <table class="table table-striped">
            ...
            <tbody>
            <?php if(is_array($menus)): foreach($menus as $key=>$menu): ?><tr>
                <td><?php echo ($menu["menu_id"]); ?></td>
                <td><?php echo ($menu["menu_name"]); ?></td>
                <td><?php echo ($menu["menu_url"]); ?></td>
                <td>
                    <?php echo $module_options_list[$menu['module_id']]; ?></td>
                <td>
                    <?php if($menu['is_show']): ?>是
                    <?php else: ?>
                    否<?php endif; ?>
                </td>
                <td><?php if($menu['father_menu'] > 0): echo ($menu["father_menu_name"]); endif; ?></td>

                <td>
                    <?php if($menu['online']): ?>在线
                    <?php else: ?>
                    已下线<?php endif; ?></td>
                <td>
                    <?php if($menu['shortcut_allowed']): ?>允许
                    <?php else: ?>
                    不允许<?php endif; ?>
                </td>
                ...
                <td>
                    <a href="<?php echo U('Menu/edit',array('menu_id'=>$menu['menu_id']));?>" title= "修改" ><i class="icon-pencil">修改</i></a>
                    <?php if($menu['menu_id'] > 100): ?><a data-toggle="modal" href="#myModal" title= "删除" ><i class="icon-remove" goto="<?php echo U('Menu/del',array('menu_id'=>$menu['menu_id']));?>" >删除</i></a><?php endif; ?>
                </td>
